...
void uiTableModelRowInserted(uiTableModel *m, int newIndex)
{
    ...
    std::vector<uiTable *>::iterator ti;
    for (ti = m->tables->begin(); ti != m->tables->end(); ++ti)
    {
        uiTable *t = *ti;
#else
	for (auto t : *(m->tables)) {
...
#if defined(_MSC_VER) && _MSC_VER < 1800
        for (std::map<std::pair<int, int>, LONG>::iterator mi = t->indeterminatePositions->begin(); mi != t->indeterminatePositions->end(); ++mi)
        {
			mi->second++;
			// TODO check errors
			SendMessageW(hwnd, LVM_UPDATE, (WPARAM) (mi->first.first), 0);
        }            
#else
...
static void uiTableDestroy(uiControl *c)
{
	...
	// free the columns
#if defined(_MSC_VER) && _MSC_VER < 1800
    std::vector<uiprivTableColumnParams *>::iterator ci;
    for (ci = t->columns->begin(); ci != t->columns->end(); ++ci)
    {
        uiprivTableColumnParams *col = *ci;
#else
