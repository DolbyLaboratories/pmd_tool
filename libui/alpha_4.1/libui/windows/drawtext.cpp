...
uiDrawTextLayout *uiDrawNewTextLayout(uiDrawTextLayoutParams *p)
{
    ...
    switch (p->Align)
    {
    case uiDrawTextAlignLeft:   hr = tl->format->SetTextAlignment(DWRITE_TEXT_ALIGNMENT_LEADING); break;
    case uiDrawTextAlignCenter: hr = tl->format->SetTextAlignment(DWRITE_TEXT_ALIGNMENT_CENTER); break;
    case uiDrawTextAlignRight:  hr = tl->format->SetTextAlignment(DWRITE_TEXT_ALIGNMENT_TRAILING); break;
    }
#endif
	if (hr != S_OK)
...
void uiDrawFreeTextLayout(uiDrawTextLayout *tl)
{
    ...
    std::vector<struct drawTextBackgroundParams *>::iterator bi;
    for (bi = tl->backgroundParams->begin(); bi != tl->backgroundParams->end(); ++bi) {
        struct drawTextBackgroundParams *p = *bi;
		uiprivFree(p);
    }
