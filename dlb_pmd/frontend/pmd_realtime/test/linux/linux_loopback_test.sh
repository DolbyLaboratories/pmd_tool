#!/bin/bash
#
# simple loop test: play a file into a loopback device, and read it back (no metadata,
# pure audio only)
#
...
#
# Note also that this file assumes a 6-channel file calls '6.wav'

PLAYOUT=`./pmd_realtime_release | grep hw:10,0 | awk '{print $2}' | cut -c-2`
CAPIN=`./pmd_realtime_release | grep hw:10,1 | awk '{print $2}' | cut -c-2`

./pmd_realtime_release play 6.wav -od $((10#$PLAYOUT)) &
PLAY_PID=$!
sleep 1

./pmd_realtime_release capture cap.wav -id $((10#$CAPIN)) &
CAP_PID=$!
